...
unique_mirnas<-unique(acc_metrics$subset_label)
unique_mirnas<-data.frame(miRNAs=unique_mirnas)
for(i in spec_metrics){
  t1<-acc_metrics[acc_metrics$metric==i&acc_metrics$Partition=="Test",] %>%
    select("subset_label",".")
  colnames(t1)[2]<-i
  unique_mirnas<-merge(unique_mirnas,t1,by.x="miRNAs",by.y="subset_label")
}

mirna_model<-acc_metrics %>% select(subset_label,sample_type) %>% unique()
unique_mirnas<-merge(unique_mirnas,mirna_model,by.x="miRNAs",by.y="subset_label") %>% 
  arrange(sample_type)
