...
plot_gini<-function(training_fit,label_m){
  ...
  sel_color<-"#395C6B"
  write.csv(imp,paste0(output_path,"gini_scores_",label_m,".csv"),row.names = F)
  if(nrow(imp)>30){
    imp<-imp[1:25,]
  }
  ...
  jpeg(paste0(output_path,"rf_",label_m,".jpg"),width=6,height=ifelse(nrow(imp)>10,6,2),units="in",res=300)
...
mirna.model <- function(train_df,test_df,coldata,model.type="rf",
  ...
  for(m1 in seq(1,nrow(track_sub))){
    ...
    track_acc<-rbind.data.frame(track_acc,temp_acc)
    plot_gini(training_fit,track_sub$num[m1])
